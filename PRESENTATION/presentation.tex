...
\begin{frame}{The Problem}
    ...
    \begin{tikzpicture}[
        ...
        \node[stat, draw=accentblue] (s2) at (4.6,0) {\$371B};
        \node[stat, draw=mutedblue] (s3) at (9.2,0) {40\%+};
        \node[lbl, below=6pt of s1] {of crashes are intersection-related};
        \node[lbl, below=6pt of s2] {annual societal cost of intersection crashes};
        \node[lbl, below=6pt of s3] {of collisions occur at intersections in International Context};
    \end{tikzpicture}
    \vspace{18pt}
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \begin{itemize}
                \item Traffic lights control at the \alert{lane-group} level, not per vehicle
...
\begin{frame}{Motivation: AV vs.\ CAV}
    ...
    \begin{tikzpicture}[
        ...
        \node[veh, right=3.0cm of av, draw=accentblue, fill=accentblue!10] (cav) {\textbf{CAV}\\[3pt] AV + V2X communication\\[4pt]\scriptsize Shares position, speed, \alert{intent} beyond line-of-sight};
...
\begin{frame}{Core Challenge}
    \begin{center}
        \Large How should an ego vehicle decide, every timestep, to \alert{go} or \alert{yield} among randomly-behaving vehicles --- safely, efficiently, reliably?
    \end{center}
    \vspace{12pt}
    \begin{itemize}
        \item Simultaneous, uncertain decisions among interacting vehicles
        \item No reservation protocol --- right-of-way is inferred, not commanded
        \item Must generalize across density, driving style, and (eventually) geometry
    \end{itemize}
\end{frame}
...
\begin{frame}{RL Literature Thread II: Graph Representations \& Cooperative AIM}
    ...
    \begin{tabularx}{\textwidth}{@{}l X X@{}}
        ...
        Klimke et al.\ (2022) & Edge-augmented RGCN + TD3, controls \emph{all} vehicles & \alert{True cooperative AIM} \\
...
\begin{frame}{Disclaimer: Scope of This Work}
    ...
    \begin{itemize}
        \item Original source code \alert{not released}
        \item Some hyperparameters / reward constants only partially specified
        \item Reproduction is faithful, but \alert{not byte-for-byte}
    \end{itemize}
\end{frame}
...
\begin{frame}{Research Progression: Three Milestones}
    ...
    \begin{tikzpicture}[
        ...
        \node[box, below=of m1] (m2) {
            ...
            \textit{Result:} found left-turn vulnerability; fixed eval bug
        };
        \node[box, below=of m2, fill=accentblue!10, draw=accentblue] (m3) {
            \textbf{M3 --- Phase 2 Exact Reproduction}\\
            Discrete SAC (twin critics, auto-tuned $\alpha$), 50k episodes\\
            \textit{Result:} 97.8\% success, 0\% collisions (1{,}050 tests)
        };
...
\begin{frame}{Action Space \& Reward Function}
    \begin{columns}[T]
        \begin{column}{0.42\textwidth}
            ...
            \]
            \small Lateral control is a fixed PD controller --- \alert{RL controls only longitudinal speed}.
        \end{column}
...
\begin{frame}{Result 1: Success \& Collision Rate}
    ...
    \small \alert{97.8\% success, 0\% collisions} over 1{,}050 episodes --- exceeds the paper's 97.4\%/2.6\%.
\end{frame}
...
\begin{frame}{Challenges and Limitations}
    ...
    \begin{tikzpicture}[
        box/.style={
            draw=navy, thick, rounded corners=4pt,
            fill=blockbodyblue,
            text width=4.3cm, minimum height=2.3cm,
            align=left, inner sep=6pt
        }
    ]
        % Row 1
        \node[box] (c1) at (-4.6, 1.45) {
            \textbf{\color{blockheaderblue}\footnotesize Incomplete Specification}\\[4pt]
            \scriptsize Original paper code \& constants unreleased; required extensive parameter inference.
        };
        \node[box] (c2) at (0, 1.45) {
            \textbf{\color{blockheaderblue}\footnotesize Multi-Ego Evaluation Bug}\\[4pt]
            \scriptsize Caught and resolved an evaluation reward attribution bug during Milestone~2.
        };
        \node[box] (c3) at (4.6, 1.45) {
            \textbf{\color{blockheaderblue}\footnotesize Partial Coverage}\\[4pt]
            \scriptsize Validated 2 of 8 paper configurations; remaining 6 ready for future extension.
        };

        % Row 2
        \node[box] (c4) at (-4.6, -1.35) {
            \textbf{\color{blockheaderblue}\footnotesize Hardware Bottleneck}\\[4pt]
            \scriptsize Hybrid-CPU thread-sync stalls resolved via physical P-core affinity pinning ($3.5\times$ speedup).
        };
        \node[box] (c5) at (0, -1.35) {
            \textbf{\color{blockheaderblue}\footnotesize Sim-to-Sim Physics Gap}\\[4pt]
            \scriptsize Simulator collision kinematics diverge between \texttt{highway-env} and paper platform.
        };
        \node[box] (c6) at (4.6, -1.35) {
            \textbf{\color{blockheaderblue}\footnotesize Single Geometry}\\[4pt]
            \scriptsize Validated solely on standard 4-way intersection; motivation for geometry-agnostic model.
        };
    \end{tikzpicture}
\end{frame}
...
\begin{frame}{Proposed Novelty: Geometry-Agnostic Decision-Making}
    \begin{alertblock}{Core direction}
        \small Every model above is trained/evaluated on \emph{one} fixed 4-way geometry --- real deployments include T-junctions, merges, roundabouts.
    \end{alertblock}
    \vspace{8pt}
    \textbf{Goal:} a policy that does not depend heavily on its training geometry.
    \begin{itemize}
        \item Evaluate: 4-way, T-junction, ramp merge, roundabout
        \item Measure transfer of right-of-way reasoning, not just re-training
    \end{itemize}
\end{frame}
...
\begin{frame}{Proposed Project Timeline}
    ...
    \begin{tikzpicture}[
        ...
        \foreach \m [count=\i from 0] in {Month 1, Month 2, Month 3, Month 4, Month 5, Month 6} {
            \node[monthhdr] at (\i*1.6 + 0.8, 2.7) {\m};
            \draw[gridline] (\i*1.6, -2.6) -- (\i*1.6, 2.7);
        }
        \draw[gridline] (9.6, -2.6) -- (9.6, 2.7);
        \draw[draw=mainblue, thick] (0, 2.65) -- (9.6, 2.65);

        % Task 1
        \node[task] at (-0.2, 2.1) {M1--M3 Baseline \& SAC Reproduction};
        \fill[blockheaderblue, bar] (0.05, 1.9) rectangle (3.15, 2.3) node[midway, font=\tiny\bfseries, text=white] {Done \checkmark};

        % Task 2
        \node[task] at (-0.2, 1.3) {Remaining Paper Configs (C--H)};
        \fill[accentblue!80, bar] (3.25, 1.1) rectangle (5.55, 1.5) node[midway, font=\tiny\bfseries, text=white] {Benchmarking};

        % Task 3
        \node[task] at (-0.2, 0.5) {Multi-Geometry Synthesis (T/Merge/Round)};
        \fill[accentblue!90, bar] (4.05, 0.3) rectangle (6.85, 0.7) node[midway, font=\tiny\bfseries, text=white] {Scene Building};

        % Task 4
        \node[task] at (-0.2, -0.3) {Geometry-Agnostic Transfer Evaluation};
        \fill[accentblue, bar] (5.65, -0.5) rectangle (8.25, -0.1) node[midway, font=\tiny\bfseries, text=white] {Evaluation};

        % Task 5
        \node[task] at (-0.2, -1.1) {Cooperative Multi-Agent / V2V Extension};
        \fill[accentblue!70, bar] (6.85, -1.3) rectangle (9.15, -0.9) node[midway, font=\tiny\bfseries, text=white] {Extension};

        % Task 6
        \node[task] at (-0.2, -1.9) {Thesis Documentation \& Defense};
        \fill[mainblue!85, bar] (7.25, -2.1) rectangle (9.55, -1.7) node[midway, font=\tiny\bfseries, text=white] {Defense};

        % Current Timeline Marker
        \draw[draw=red!80!black, line width=1.2pt, dashed] (3.2, -2.5) -- (3.2, 2.9)
            node[above, font=\tiny\bfseries, text=red!80!black] {Current Status};

        % Legend
        \node[anchor=north, font=\tiny] at (4.8, -2.6) {
            \tikz\fill[blockheaderblue, rounded corners=1pt] (0,0) rectangle (0.3, 0.15); Completed \quad
            \tikz\fill[accentblue, rounded corners=1pt] (0,0) rectangle (0.3, 0.15); Proposed Milestones \quad
            \tikz\draw[red!80!black, line width=1pt, dashed] (0,0) -- (0.4, 0); Current Progress
        };
    \end{tikzpicture}%
    }
\end{frame}
...
\begin{frame}{Conclusion}
    \begin{itemize}\setlength{\itemsep}{8pt}
        \item Full DRL pipeline validated across three progressive milestones
        \item 1:1 Discrete SAC reproduction, matching and exceeding published benchmarks
        \item Performance gap traced to simulator collision physics and specification gaming
        \item Concrete path and experimental framework established toward \alert{geometry-agnostic} decision-making
    \end{itemize}
\end{frame}
