...
\begin{frame}{Introduction}
    % [Template slide: Add Introduction content here]
\end{frame}

\begin{frame}{Why Unsignalized Intersections?}
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            \textbf{The problem with traffic lights}
            \begin{itemize}
                \item Fixed or semi-adaptive timing $\rightarrow$ cars stop even when the road is clear
        ...
        \begin{column}{0.45\textwidth}
            \begin{block}{Autonomous Intersection Management (AIM)}
                Signal-free coordination: connected \& automated vehicles negotiate crossing order and speed \emph{individually}, like dancers threading through a shared space instead of waiting for a shared green light.
            \end{block}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Why CAV, Not Just AV?}
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \textbf{Autonomous Vehicle (AV)}
            \begin{itemize}
                \item Relies purely on onboard sensing (camera, lidar, radar)
                \item No information sharing with other vehicles / infrastructure
                \item Limited by sensor range and occlusion
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \textbf{Connected \& Automated Vehicle (CAV)}
            \begin{itemize}
                \item V2X communication: V2V (vehicle-to-vehicle) \& V2I (vehicle-to-infrastructure)
                \item Shares position, speed, and \alert{intent} beyond line-of-sight
                \item Intent / communication data can be fed directly into the decision policy
            \end{itemize}
        \end{column}
    \end{columns}
    \vspace{8pt}
    \begin{alertblock}{Key idea}
        V2X lets a CAV reason about \emph{what other vehicles intend to do}, not just where they currently are — this is exactly the gap our RL state design (task-intent feature $\mu_a$) is built to exploit.
    \end{alertblock}
\end{frame}
